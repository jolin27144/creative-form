...
function getCascaderValue(address, area, code = 0, str = "") {
  ...
  for (let i in area) {
    if (area[i].value === address[code]) {
      str =
        (str ? str + " > " : "") +
        getCascaderValue(address, area[i].children, code + 1, area[i].label);
      break;
    }
  }
  return str;
...
const getObjValue = (ele, obj) => {
  ...
  const arr = ["input", "text", "datepicker"];
  ...
  if (ele == "select" || ele == "radio") {
    return (getSelectValue(value, items)[0] || {}).label_name;
  }

  if (ele == "checkbox") {
    let str = "";
    getCheckBoxValue(value, items).forEach(v => {
      str += v.label_name + ",";
    });
    return str.substring(0, str.length - 1);
  }

  return (items[value] || {}).label_name || "";
};
